...
💌 I am a Whatsapp BOTz
Which was Created by *FACompanyID*
I Successfully Launched Officially on:
_12 Desember 2020_
I Developed by: 
-. Farrell Athar R
-. Natx-C
...
`.trim()
        ...
        melcanz.sendMessage(m.chat, img, mType.image, { quoted: freply('FA-BOTz', img), caption: menu })
        break
//━━━━━━━━━━━━━━━[CASE MAKER MENU]━━━━━━━━━━━━━━━━━//
case prefix + 'neon':
        if (!text) throw `[ ? ] Teksnya Manaaa Sloerr?`
        apii = global.API('mel', '/textpro/neon', { text }, 'apikey')
        buffer = await getBuffer(apii)
        melcanz.sendMessage(m.chat, buffer, 'imageMessage', { quoted: m, caption: '[ ✓ ] Success...' })
        break
...
case prefix + 'tahta':
      case prefix + 'hartatahta':
        if (!args[0]) return m.reply('[ ? ] Teksnya Manaaa Sloerr?')
...
case prefix + 'stiker':
      ...
      case prefix + 'stickergif':
        ...
        if (!/image|video/.test(json.mtype)) return m.reply(`[ ! ] Reply Video/Gambar dengan Caption *${prefix + command}*!`)
...
case prefix + 'nulis':
        ...
        melcanz.sendMessage(m.chat, { url: result }, mType.image, { quoted: m, fileLength: 999999999999999, caption: '[ ✓ ] Success...' })
        break
//━━━━━━━━━━━━━━━[CASE OWNER MENU]━━━━━━━━━━━━━━━━━//
case prefix + 'block':
      case prefix + 'blok':
...
case prefix + 'unblock':
      case prefix + 'unblok':
        //if (!m.isGroup) return m.reply('[ ! ] Perintah ini Khusus di dalam Group!')
        //if (!isAdmin) return m.reply('[ ! ] Perintah ini Khusus Admin Group!')
        if (!isOwner) return
        isQuod = m.quoted ? [m.quoted.sender] : m.mentionedJid
        if (!isQuod[0]) return m.reply('[ ! ] Tag Member yang ingin di Block!')
        isQuod.map(a => {
        ...
        if (!/image/.test(ye.mtype)) return m.reply('[ ? ] Fotonya mana Sloerr??')
...
case prefix + 'setthumb':
        ...
        melcanz.reply(m.chat, '[ ✓ ] Successfully Changing Thumbnails...', freply(null, buffer))
        break
case prefix + 'setname':
        if (!isOwner) return
        if (!args[0]) return m.reply('[ ? ] Teksnya Manaaa Sloerr?')
...
case prefix + 'join':
        if (!isOwner) return
        if (!args[0]) return m.reply('[ ? ] Linknya mana Sloerr?')
...
case '$':
        if (!text) return
        if (!isOwner) throw `[ ! ] Perintah ini Khusus Owner BOTz yaa!!!!`
        m.reply('```[⏳] Executing...```')
...
case '>':
 ...
 return ${args.join(' ')}
        ...
        break
case prefix + 'public':
        if (!isOwner) throw `[ ! ] Perintah ini Khusus Owner BOTz!`
      ...
      case prefix + 'self':
        ...
        break
case prefix + 'bcgc':
        if (!isOwner) return m.reply('[ ! ] Perintah ini Khusus Owner BOTz!')
        if (!args[0]) return m.reply('[ ? ] Teksnya mana Om!')
        ...
        melcanz.reply(m.chat, `_[⏳] Mengirim Pesan Broadcast ke ${chats.length} group_`, m)
        break
case prefix + 'ohidetag':
        if (!m.isGroup) return m.reply('[ ! ] Perintah ini Khusus di dalam Group!')
        if (!isOwner) return m.reply('[ ! ] Perintah ini Khusus Admin Group!')
...
case prefix + 'kick':
        if (!m.isGroup) return m.reply('[ ! ] Perintah ini Khusus di dalam Group!')
        if (!isAdmin) return m.reply('[ ! ] Perintah ini Khusus Admin Group!')
        if (!isBotAdm) return m.reply('[ ! ] Jadikan BOTz sebagai Admin Terlebih Dahulu!')
        isQuod = m.quoted ? [m.quoted.sender] : m.mentionedJid
        if (!isQuod[0]) return m.reply('[ ! ] Tag Member yang ingin di Kick!')
        isQuod.map(a => {
...
case prefix + 'add':
        ...
        if (isQuod.length == 0) return m.reply(`[ ? ] Siapa yang ingin di Add?`)
        ...
        for (let user of response.participants.filter(user => Object.values(user)[0].code == 403)) {
          ...
          teks = `[⏳] Proses Mengundang @${jid.split('@')[0]} Menggunakan Undangan Group...`
          m.reply(teks, null, {
            contextInfo: {
...
case prefix + 'antidelete':
        ...
        if (!args[0]) {
          ...
          const buttonsMessage = {
            contentText: `[ • ] Pilihlah Enable atau Disable`.trim(),
            footerText: `🔰 ${melcanz.user.name} By FarrellDEV`,
            buttons: buttons,
            headerType: 1
          }
          ...
          if (antidelete.includes(m.chat)) return m.reply('[ ! ] ANTIDELETE Feature telah Diaktifkan Sebelumnya')
          antidelete.push(m.chat)
          fs.writeFileSync('./database/chat/antidelete.json', JSON.stringify(antidelete, null, 2))
          m.reply('[ ✓ ] Sukses Mengaktifkan ANTIDELETE pada Group ini....')
        } else if (/off|disable/gi.test(args[0])) {
          index = antidelete.indexOf(m.chat)
          antidelete.splice(index, 1)
          m.reply('[ ✓ ] Sukses Menonaktifkan ANTIDELETE pada Group ini....')
          fs.writeFileSync('./database/chat/antidelete.json', JSON.stringify(antidelete, null, 2))
      ...
      case prefix + 'welcome':
        ...
        if (!args[0]) {
          ...
          m.reply('[ ✓ ] Sukses Mengaktifkan Feature Welcome di Group Ini....')
        } else if (/off|disable/gi.test(args[0])) {
          index = welcome.indexOf(m.chat)
          welcome.splice(index, 1)
          m.reply('[ ✓ ] Sukses Menonaktifkan Feature Welcome di Group Ini....')
          fs.writeFileSync('./database/chat/welcome.json', JSON.stringify(welcome, null, 2))
        } else m.reply('[ ! ] Pilih Enable atau Disable')
        break
      case prefix + 'left':
        ...
        if (!args[0]) {
          ...
          if (left.includes(m.chat)) return m.reply('[ ! ] Goodbye Feature telah Diaktifkan Sebelumnya')
          left.push(m.chat)
          fs.writeFileSync('./database/chat/left.json', JSON.stringify(left, null, 2))
          m.reply('[ ✓ ] Sukses Mengaktifkan Goodbye Feature di Group ini....')
        } else if (/off|disable/gi.test(args[0])) {
          index = left.indexOf(m.chat)
          left.splice(index, 1)
          m.reply('[ ✓ ] Sukses Menonaktifkan Goodbye Feature di Group ini....')
          fs.writeFileSync('./database/chat/left.json', JSON.stringify(left, null, 2))
        } else m.reply('[ ! ] Pilih Enable atau Disable')
        break
case prefix + 'promote':
...
case 'subject':
        ...
        break
case prefix+'sider':
if (!m.quoted) throw `[ ! ] Reply Chat BOTz!`
if (!m.quoted.fromMe) throw `[ ! ] Reply Chat BOTz!`
qtss = m.quoted
...
case prefix + 'pinterest':
      ...
      case prefix + 'wikipedia':
        if (!q) return m.reply(`*- EXAMPLES OF USE -*\n${prefix}wiki google`)
        m.reply(`_*[⏳] Tunggu Permintaan Anda sedang Diproses..._*`)
...
//━━━━━━━━━━━━━━━[CASE EDUCATION MENU]━━━━━━━━━━━━━━━━━//
case prefix + 'brainly':
        if (!q) return m.reply('[ ? ] Soalnya Apa Kak?')
        m.reply('*_[⏳] Tunggu Permintaan Anda sedang Diproses..._*')
        brainly(q, 10)
          .then(async bren => {
            teks = '*「 _BRAINLY_ 」*\n\n'
...
case prefix + 'calc':
        melcanz.updatePresence(m.chat, 'composing')
        if (!text) return m.reply(`[ ? ] Teksnya Mana Sloerr!!!`)
        var val = text
...
case prefix + 'togif':
      ...
      case prefix + 'tomp4':
        ...
        } else throw '[ ! ] Reply Stiker / Audio yang ingin dijadikan Video!'
        break
//━━━━━━━━━━━━━━━[CASE RANDOM MENU]━━━━━━━━━━━━━━━━━//
...
case prefix + 'loli':
        ...
        melcanz.sendMessage(m.chat, { url }, mType.image, { quoted: m, caption: '[🌑] Lolinya bang...' })
        break
      case prefix + 'lolimaker':
        if (!text) throw `[ ? ] Teksnya Manaaa Sloerr??`
        apii = global.API('mel', '/lolimaker', { text }, 'apikey')
        buffer = await getBuffer(apii)
        melcanz.sendMessage(m.chat, buffer, 'imageMessage', { quoted: m, caption: '[🌑] Neh bang lolinya :v' })
        break
      case prefix + 'remlogo':
        if (!text) throw `[ ? ] Teksnya Manaaa Sloerr??`
        apii = global.API('mel', '/rem', { text }, 'apikey')
        buffer = await getBuffer(apii)
        melcanz.sendMessage(m.chat, buffer, 'imageMessage', { quoted: m, caption: '[🌑] Neh bang logo remnya :v' })
        break
      case prefix + 'kanekilogo':
      case prefix + 'kanekimaker':
        if (!text) throw `[ ? ] Teksnya Manaa Sloerr??`
        apii = global.API('mel', '/kaneki', { text }, 'apikey')
        buffer = await getBuffer(apii)
        melcanz.sendMessage(m.chat, buffer, 'imageMessage', { quoted: m, caption: '[🌑] Neh bang logo kanekinya :v' })
        break
      case prefix + 'sadboy':
        if (!text) throw `*- NO TEXT DETECTED -*\nEx : ${command} farrellganz|ganz`
        var [tek, tek2] = text.split('|')
      ...
      case prefix + 'nekomaker':
        ...
        buffer = await getBuffer(apii)
        melcanz.sendMessage(m.chat, buffer, 'imageMessage', { quoted: m, caption: '[ : ] Neh bang logo nekonya :v' })
        break
//━━━━━━━━━━━━━━━[CASE DEWASA MENU]━━━━━━━━━━━━━━━━━//
case prefix + 'lewd':
        m.reply(`_*[⏳] Tunggu Permintaan Anda sedang diproses..*_`)
...
case prefix + 'sc':
      case prefix + 'script':
        m.reply(`This Script Private!`)
        break
case prefix + 'sewabot': {
        let qrisnya = fs.readFileSync('thumb/itsuki.jpg')
        let teksewa = `
_❀ List Harga Sewa BOTz by FACompanyID_
--------------------------
_1 MINGGU = 7K _
_1 BULAN = 15K _
_2 BULAN = 20K _
...
_MINAT? HUBUNGI_ *wa.me/6285807231447*`
...
if (!m.quoted.fromMe) throw `[ ! ] Tidak Bisa menghapus pesan orang lain!`
m.quoted.delete()
break
case prefix+'regist':
if (isRegist) throw `[ ! ] Kamu telah Regist Sebelumnya!`
...
•> UNREG: ${shortlink.result.url}`.trim()
...
if (!args[0]) return
if (args[0] !== m.sender.split('@')[0]) throw `[ ! ] Nomor Anda Tidak Valid!` 
